...
이야기는 Xialong으로 요청하며, API가 구독 등급의 모델 사용 권한 부족을 명시하면 GLM-4.6으로 자동 전환합니다. Scroll·Tablet에서도 별도 모델 설정 없이 사용할 수 있습니다. 같은 키의 권한 거절은 10분 동안 기억하며 다른 키에는 적용하지 않습니다. 입력 길이 초과·인증·통신 오류에는 이 전환을 적용하지 않습니다. GLM-4.6도 실패하면 오류를 보존하고 기존 재시도 기능을 사용합니다. 모델별 이야기 성향과 구독 등급별 문맥 한도는 다를 수 있습니다.
