...
function notify(message){$('middleMessage').textContent=message;if($('batchStatus'))$('batchStatus').textContent=message}
...
async function pollJob(){clearTimeout(pollTimer);try{const j=await api('/api/story/jobs/'+currentJob.id);currentJob=j;notify(j.message||j.status);state.busy=['running','queued'].includes(j.status);$('middleJobActions').hidden=state.busy||j.status==='complete';$('middleRetry').disabled=!j.retryable;$('middleRestart').disabled=!j.restartable;updateControls();if(state.busy)pollTimer=setTimeout(pollJob,1200);else{if(j.status==='complete'){if(j.kind==='compose'){state.selected.clear();saveSelection()}clearKeys.forEach(storageRemove);clearKeys=[];localStorage.removeItem('naimanga-middle-job');await refreshProject();if(batch)await nextBatch()}else if(batch){stopBatch();notify((j.message||'그림 생성 실패')+' 한꺼번에 생성을 중단했습니다. 완료된 그림은 유지됩니다.')}}}catch(e){stopBatch();state.busy=false;updateControls();notify('진행 확인에 실패했습니다. 새로고침하면 저장된 작업을 다시 확인합니다. '+e.message)}}
...
window.ComicMiddle={render,saveSettings(payload,key){return launchJob(projectPath()+'/settings',payload,'현재 이야기 설정을 저장하고 있습니다.',{clear:[key]});},renderPage(id,payload={}){return startRender(state.project.pages.find(p=>p.id===id),payload);},setBusy(value){externalBusy=value;if(state.project)updateControls()},resume(){const saved=localStorage.getItem('naimanga-middle-job');batch=readDraft('page-batch');if(batch?.project_id!==state.project?.id)batch=null;if(saved){try{currentJob=JSON.parse(saved);clearKeys=currentJob.clearKeys||[];if(currentJob.project_id===state.project?.id)pollJob()}catch{}}}};
